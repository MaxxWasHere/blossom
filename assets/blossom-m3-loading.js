/**
 * Material 3 Expressive loading indicator — canvas morph + spring rotation.
 * Shape geometry and motion ported from Android material-components-android
 * (Apache 2.0); adapted for Blossom vanilla assets.
 */
(function () {
  "use strict";
  ...
  class M3Animator {
    constructor() {
      ...
      this.rotation = 0;
      this.morph = 0;
      this.spring = new Spring(200, 0.6);
  ...
  function drawIndicator(ctx, cssSize, points, rotation, options) {
    ...
    ctx.closePath();
    ctx.fillStyle = options.color || "currentColor";
    ctx.fill();
    ctx.restore();
  }
  ...
  function themeRoot(hostEl) {
    return (
      hostEl.closest(".blossom-loading-overlay, .bootstrap-splash-root, .window-frame") ||
      document.documentElement
    );
  }

  function resolveColors(hostEl) {
    const cs = getComputedStyle(themeRoot(hostEl));
    const accent = cs.getPropertyValue("--accent").trim();
    const container =
      cs.getPropertyValue("--md-sys-color-primary-container").trim() ||
      cs.getPropertyValue("--bg-input").trim() ||
      "rgba(0,0,0,0.12)";
    return {
      color: accent || "#e891a8",
  ...
  function mount(hostEl, options) {
    ...
    canvas.className = "blossom-m3-loading-canvas";
    ...
    const loop = (ts) => {
      if (!instances.has(hostEl)) return;
      const colors = resolveColors(hostEl);
      const reduced = reducedMotion();

      if (reduced) {
        anim.paused = true;
        if (anim.morph === 0) anim.morph = 6;
        pulsePhase += 0.02;
        const alpha = 0.72 + Math.sin(pulsePhase) * 0.12;
        ctx.globalAlpha = alpha;
        drawIndicator(ctx, size, getMorphedShape(6), 0, {
          color: colors.color,
          contained,
          containerColor: colors.containerColor,
          sizeRatio: opts.sizeRatio || 0.79,
        });
        ctx.globalAlpha = 1;
      } else {
        anim.paused = false;
        anim.update(ts);
        ctx.globalAlpha = 1;
        drawIndicator(ctx, size, getMorphedShape(anim.morph), anim.rotation, {
          color: opts.color || colors.color,
          contained,
          containerColor: opts.containerColor || colors.containerColor,
          sizeRatio: opts.sizeRatio || 0.79,
        });
      }
    ...
    rafId = requestAnimationFrame(loop);
    instances.set(hostEl, { canvas, rafId, anim });
  }
  ...
  function sync(hostEl, visible) {
    if (!hostEl) return;
    if (visible && !hostEl.hidden && hostEl.offsetParent !== null) {
      if (!instances.has(hostEl)) mount(hostEl);
    } else {
      unmount(hostEl);
    }
  }

  window.BlossomM3Loading = { mount, unmount, sync };
})();
