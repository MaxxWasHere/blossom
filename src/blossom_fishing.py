...
def _log_opencv_missing(log_prefix: str = "[Fishing]") -> None:
    global _CV2_MISSING_LOGGED
    if _CV2_MISSING_LOGGED or _cv2_module() is not None:
        return
...
def _load_config_file() -> dict[str, Any]:
    try:
        from blossom_dirs import APP_CONFIG_PATH
...
def detect_colour(
    ...
    *,
    ...
    in_range = np.all((bgr >= lower_bound) & (bgr <= upper_bound), axis=2)
...
def _run_walk_to_fish_path(
    *,
    ...
    if replay_movement_path_cb is not None:
        try:
            if replay_movement_path_cb("fishing"):
                return True
        except Exception as error:
            print(f"[Fishing] fishing.json replay failed, using built-in walk: {error}")
...
def _run_walk_to_sell_fish_path(
    *,
    ...
    if replay_movement_path_cb is not None:
        try:
            if replay_movement_path_cb("fishing_sell"):
                return True
        except Exception as error:
            print(f"[Fishing] fishing_sell.json replay failed, using built-in walk: {error}")
...
def run_fishing_loop(
    *,
    ...
    _log_opencv_missing(log_prefix)
